...
- `python -m rdb2g.cli.full_experiment` 是一键编排：先生成 TTL，再（可选）导入 Neo4j，最后跑评测。
...
- 全流程（TTL -> Neo4j 导入 -> 评测）：
  - `python -m rdb2g.cli.full_experiment "<db_path>" --question-bank "docs/2026-04-09/rdb_vs_kg_eval_sample_cases_100.csv" --engine both`
- 仅评测：
